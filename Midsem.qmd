...
```{r data-load}
sust_eff <- readr::read_csv("/Users/namomac/Desktop/SURV-687/sust_eff.csv", 
...
```{r q13-model}
model13 <- lmer(math ~ female + grade + lowinc_cen + minority 
+ (1 | schoolid), data = sust_eff)
summary(model13)
```
...
# Question 14
```{r q14-model}
model14 <- lmer(math ~ female + grade + lowinc_cen + 
minority + (1 + minority | schoolid), 
data = sust_eff, control = lmerControl(optimizer = "bobyqa"))
summary(model14)
```
...
This result provides strong evidence of significant between-school variability in the effect of minority status. In other words, the **minority achievement gap differs meaningfully across schools**, which I guess is suggesting that school-level factors may influence how minority status affects academic outcomes.
...
preferred_model <- if (lr15$`Pr(>Chisq)`[2] < 0.05) 
best_model_reml else update(best_model_reml, . ~ . - female)

# This will show residuals, random effects, QQ plots, etc.
check_model(preferred_model, check = c("normality", 
"linearity", "homoscedasticity", "outliers", "random_effects"))
```

## Model Comparison

So now I compared two fitted models to assess the impact of including a fixed effect for female.
...
Given this result, occam's razor, the simpler model i.e. without the fixed effect for female, is preferred due to its parsimony.
