...
Fit a multilevel model with `math` as the dependent variable, fixed effects for `female`, `grade`, `lowinc_cen`, and `minority`, and a random intercept for `schoolid`.
...
- The `lowinc_cen` effect (`r round(fixef(model13)["lowinc_cen"], 2)`) quantifies how math scores change as the centered percentage of low-income students at a school increases by one unit.
